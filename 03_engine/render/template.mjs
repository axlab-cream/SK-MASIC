...
function buildRefineCompositeHtml({ products, copy, styleId, sizeKey, consultant, pattern }) {
  ...
  else if (benefit) content = useRows ? `${benefitPanel}<section class="rc-list">${listRows}</section>`
    : `${benefitPanel}<section class="rc-product-grid">${products.map((p, i) => productCard(p, i)).join('')}</section>`;
  else content = useRows ? `<section class="rc-list">${listRows}${summary}</section>`
    : `<section class="rc-feature">${featured}<div class="rc-product-grid">${remaining}</div></section>${summary}`;
