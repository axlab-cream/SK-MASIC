...
function buildRefineCompositeHtml({ products, copy, styleId, sizeKey, consultant, pattern }) {
  ...
  const productCard = (p, i, hero = false) => `<article class="rc-product${hero ? ' hero' : ''}" data-slot="product-${i}" data-kind="product">
    <div class="rc-visual">${productVisual(p, st)}</div><div class="rc-product-copy">
      ...
      <b>월 ${won(shown(i))}원</b></div></article>`;
  const listRows = products.map((p, i) => `<article class="rc-row" data-slot="product-${i}" data-kind="product"><em>${String(i + 1).padStart(2, '0')}</em>
    <div class="rc-thumb">${productVisual(p, st)}</div><div><strong>${esc(p.name)}</strong><span>${esc(copy.labels?.[i] || p.color || '')}</span></div><b>월 ${won(shown(i))}원</b></article>`).join('');
  const featured = products[0] ? productCard(products[0], 0, true) : '';
  const remaining = products.slice(1).map((p, i) => productCard(p, i + 1)).join('');
  const benefitPanel = `<section class="rc-benefits" data-slot="benefits" data-kind="aux"><p>BENEFIT 01</p><strong>${products.length}종 함께 쓰면 월 ${won(Math.max(0, base - total))}원 할인</strong>
    <span><del>월 ${won(base)}원</del> → <b>월 ${won(total)}원</b></span><hr/><p>BENEFIT 02</p><strong>시작할 때 드는 비용 0원</strong>
    <div><b>● 설치비 면제</b><b>● 등록비 면제</b><b>● 무상 A/S</b></div></section>`;
  ...
  let content = '';
  if (single && price) content = `<section class="rc-pricehero" data-slot="pricehero" data-kind="aux"><span>${esc(products[0].name)} 월 렌탈료</span>${base ? `<del>월 ${won(base)}원</del>` : ''}<b>월 ${won(total)}원</b><small>약정 기간 및 할인 조건은 하단 고지문을 확인해 주세요</small></section><section class="rc-feature single-product">${featured}</section>`;
  else if (single && benefit) content = `<section class="rc-feature single-product">${featured}</section>${benefitPanel}`;
  else if (single) content = `<section class="rc-feature single-product">${featured}</section>`;
  else if (price) content = `<section class="rc-pricehero" data-slot="pricehero" data-kind="aux"><span>${products.length}종 묶음 · 월 렌탈료</span>${base ? `<del>월 ${won(base)}원</del>` : ''}<b>월 ${won(total)}원</b><small>약정 기간 및 제휴카드 조건은 하단 고지문을 확인해 주세요</small></section><section class="rc-list">${listRows}${summary}</section>`;
  else if (benefit) content = `${benefitPanel}<section class="rc-product-grid ${products.length > 6 ? 'compact' : ''}">${products.map((p, i) => productCard(p, i)).join('')}</section>`;
  else content = `<section class="rc-feature">${featured}<div class="rc-product-grid ${products.length > 6 ? 'compact' : ''}">${remaining}</div></section>${summary}`;
...
.rc-feature,.rc-list,.rc-benefits,.rc-pricehero{background:${surface};border:1px solid ${dark ? '#30313A' : '#ECECEC'};border-radius:${Math.round(S*.022)}px;padding:${Math.round(S*.024)}px}.rc-feature{display:flex;flex-direction:column;gap:${Math.round(S*.016)}px}.single-product{min-height:${Math.round(S*.34)}px;justify-content:center}.single-product .rc-product.hero{min-height:${Math.round(S*.29)}px}.single-product .hero .rc-visual{height:${Math.round(S*.25)}px}.rc-product-grid{display:grid;grid-template-columns:repeat(${products.length <= 3 ? Math.max(1, products.length - 1) : products.length <= 6 ? 3 : 4},1fr);gap:${Math.round(S*.014)}px}.rc-product-grid.compact{grid-template-columns:repeat(4,1fr);gap:${Math.round(S*.009)}px}.rc-product{min-height:${Math.round(S*.15)}px;background:${dark ? '#111217' : '#F8F8F8'};border:${variant === 'benefit-b' ? '1px solid #E9E1D8' : '0'};border-radius:${Math.round(S*.014)}px;padding:${Math.round(S*.014)}px;display:flex;align-items:center;gap:${Math.round(S*.012)}px}.rc-product.hero{min-height:${Math.round(S*.26)}px}.rc-visual{width:36%;height:${Math.round(S*.12)}px;display:flex;align-items:center;justify-content:center}.hero .rc-visual{height:${Math.round(S*.22)}px;width:38%}.pimg{max-width:100%;max-height:100%;object-fit:contain}.rc-product-copy{flex:1;min-width:0;display:flex;flex-direction:column;gap:${Math.round(S*.004)}px}.rc-product-copy strong{font-size:${Math.max(18,Math.round(S*.032))}px;line-height:1.18}.rc-product-copy span{font-size:${Math.max(14,Math.round(S*.021))}px;color:${dark ? '#B9BBC4' : '#727272'};white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.rc-product-copy b{font-size:${Math.max(18,Math.round(S*.03))}px;color:${accent}}.compact .rc-product{padding:${Math.round(S*.008)}px;display:block;text-align:center}.compact .rc-visual{width:100%;height:${Math.round(S*.09)}px}.compact .rc-product-copy strong{font-size:${Math.max(14,Math.round(S*.021))}px}.compact .rc-product-copy span{font-size:${Math.max(12,Math.round(S*.016))}px}.compact .rc-product-copy b{font-size:${Math.max(14,Math.round(S*.021))}px}
.rc-summary{display:flex;justify-content:space-between;align-items:center;border:1px solid ${dark ? '#3A3B45' : '#D9D9D9'};border-radius:${Math.round(S*.014)}px;padding:${Math.round(S*.018)}px ${Math.round(S*.022)}px;min-height:${Math.round(S*.105)}px}.rc-summary strong{font-size:${Math.max(18,Math.round(S*.03))}px}.rc-summary span{display:block;font-size:${Math.max(14,Math.round(S*.019))}px;color:${dark ? '#B9BBC4' : '#8A8A8A'};margin-top:4px}.rc-summary del{display:block;text-align:right;font-size:${Math.max(14,Math.round(S*.018))}px;color:${dark ? '#9A9BA3' : '#999'}}.rc-summary b{display:block;font-size:${Math.max(22,Math.round(S*.044))}px;color:${accent}}
.rc-pricehero{background:${variant === 'price-b' ? '#EB1235' : '#FF741D'};color:#fff;box-shadow:0 ${Math.round(S*.016)}px ${Math.round(S*.03)}px rgba(255,100,31,.18)}.rc-pricehero span,.rc-pricehero del,.rc-pricehero small{display:block;font-size:${Math.max(16,Math.round(S*.026))}px;margin-bottom:${Math.round(S*.01)}px}.rc-pricehero b{display:block;font-size:${Math.round(S*.088)}px;line-height:1.05}.rc-pricehero small{margin:0;margin-top:${Math.round(S*.014)}px}.rc-list{padding:${Math.round(S*.016)}px}.rc-row{display:grid;grid-template-columns:${Math.round(S*.045)}px ${Math.round(S*.10)}px 1fr auto;align-items:center;gap:${Math.round(S*.012)}px;min-height:${Math.round(S*.075)}px;border-bottom:1px solid ${dark ? '#34353D' : '#E8E8E8'};padding:${Math.round(S*.008)}px 0}.rc-row em{font-style:normal;color:#999;font-size:${Math.max(16,Math.round(S*.023))}px;font-weight:700}.rc-thumb{height:${Math.round(S*.06)}px;display:flex;align-items:center;justify-content:center}.rc-thumb .pimg{max-height:100%;max-width:100%}.rc-row strong{font-size:${Math.max(16,Math.round(S*.027))}px;display:block}.rc-row span{display:block;font-size:${Math.max(13,Math.round(S*.02))}px;color:#777;margin-top:2px}.rc-row>b{font-size:${Math.max(16,Math.round(S*.027))}px;color:${dark ? '#fff' : '#202020'};white-space:nowrap}
.rc-benefits{text-align:center}.rc-benefits p{font-size:${Math.max(16,Math.round(S*.022))}px;letter-spacing:.2em;color:${accent};font-weight:700;margin-bottom:${Math.round(S*.01)}px}.rc-benefits strong{display:block;font-size:${Math.max(20,Math.round(S*.04))}px}.rc-benefits>span{display:block;margin-top:${Math.round(S*.011)}px;font-size:${Math.max(16,Math.round(S*.027))}px;color:#999}.rc-benefits>span b{color:${accent}}.rc-benefits hr{border:0;border-top:1px solid #E9E9E9;margin:${Math.round(S*.018)}px 0}.rc-benefits>div{display:grid;grid-template-columns:repeat(3,1fr);margin-top:${Math.round(S*.012)}px}.rc-benefits>div b{font-size:${Math.max(16,Math.round(S*.026))}px;color:${dark ? '#fff' : '#333'}}
...
</style></head><body><div id="banner"><main class="rc ${single ? 'single' : 'composite'}"><header class="rc-header" data-slot="headline" data-kind="aux">${logo ? `<img class="rc-logo" src="${logo}" alt="SK매직">` : `<span class="rc-logo-slot">SK magic</span>`}<span class="rc-kicker">${ice ? '얼음정수기 캠페인' : benefit ? '혜택 총정리' : dark ? 'ICE IS MAGIC' : single ? '가을 한정 제안' : `${products.length}종 묶음 특가`}</span><div class="rc-tags"><span>렌탈 혜택</span><span>${ice ? '얼음정수기' : '매직 추천'}</span></div><h1>${esc(copy.headline)}</h1><p class="rc-sub">${esc(copy.subline)}</p></header>${content}<div class="rc-cta" data-slot="cta" data-kind="aux">${esc(copy.cta)}</div>${mc ? `<footer class="rc-footer" data-slot="brand" data-kind="aux"><strong>${esc(mc.name)}</strong>${mc.phone ? `<span>${esc(mc.phone)}</span>` : ''}</footer>` : ''}</main><div class="notice aux" data-slot="notice">${esc(noticeText)}</div></div><script>window.__LAYOUT=${JSON.stringify({w,h,S,FMIN:fmin(w,h),NMIN:noticeMin(w,h),sz:safeZone(w,h,Math.round(S*.06)),pad:Math.round(S*QA.exclusionPadRatio),qa:QA,dark,bg:canvas})};${LAYOUT_JS}</script></body></html>`;
}
