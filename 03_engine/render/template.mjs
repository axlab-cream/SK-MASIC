...
function buildRefineCompositeHtml({ products, copy, styleId, sizeKey, consultant, pattern }) {
  ...
  let content = '';
  if (price) content = `<section class="rc-pricehero" data-slot="pricehero" data-kind="aux"><span>${products.length}종 묶음 · 월 렌탈료</span>${base ? `<del>월 ${won(base)}원</del>` : ''}<b>월 ${won(total)}원</b><small>약정 기간 및 제휴카드 조건은 하단 고지문을 확인해 주세요</small></section><section class="rc-list">${listRows}${summary}</section>`;
...
</style></head><body><div id="banner"><main class="rc"><header class="rc-header" data-slot="headline" data-kind="aux">${logo ? `<img class="rc-logo" src="${logo}" alt="SK매직">` : `<span class="rc-logo-slot">SK magic</span>`}<span class="rc-kicker">${ice ? '얼음정수기 캠페인' : benefit ? '혜택 총정리' : dark ? 'PREMIUM LINE-UP' : `${products.length}종 묶음 특가`}</span><div class="rc-tags"><span>렌탈 혜택</span><span>${ice ? '얼음정수기' : '매직 추천'}</span></div><h1>${esc(copy.headline)}</h1><p class="rc-sub">${esc(copy.subline)}</p></header>${content}<div class="rc-cta" data-slot="cta" data-kind="aux">${esc(copy.cta)}</div>${mc ? `<footer class="rc-footer" data-slot="brand" data-kind="aux"><strong>${esc(mc.name)}</strong>${mc.phone ? `<span>${esc(mc.phone)}</span>` : ''}</footer>` : ''}</main><div class="notice aux" data-slot="notice">${esc(noticeText)}</div></div><script>window.__LAYOUT=${JSON.stringify({w,h,S,FMIN:fmin(w,h),NMIN:noticeMin(w,h),sz:safeZone(w,h,Math.round(S*.06)),pad:Math.round(S*QA.exclusionPadRatio),qa:QA,dark,bg:canvas})};${LAYOUT_JS}</script></body></html>`;
}
