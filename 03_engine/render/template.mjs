...
function buildRefineCompositeHtml({ products, copy, styleId, sizeKey, consultant, pattern }) {
  ...
  const campaignBackground = ice && existsSync(ICE_BACKGROUND)
    ? `#DDF3FF url('${pathToFileURL(ICE_BACKGROUND).href}') center / cover no-repeat` : canvas;
...
#banner{position:relative;width:${w}px;height:${h}px;overflow:hidden;background:${campaignBackground};color:${dark ? '#fff' : '#202020'};padding:${Math.round(S*.06)}px ${Math.round(S*.06)}px ${Math.round(S*.12)}px}
